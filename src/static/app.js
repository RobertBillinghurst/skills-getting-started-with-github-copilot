document.addEventListener("DOMContentLoaded", () => {
  ...
  function getParticipantsMarkup(activityName, participants) {
    if (!participants.length) {
      return '<li class="participants-empty">No participants yet. Be the first to join.</li>';
    }
        ...
        activityCard.innerHTML = `
          ...
          <div class="participants-section">
            ...
            <ul class="participants-list">
              ${getParticipantsMarkup(name, details.participants)}
            </ul>
          </div>
        `;
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      const result = await unregisterParticipant(activityName, email);
      messageDiv.textContent = result.message;
      messageDiv.className = "success";
      messageDiv.classList.remove("hidden");
      await fetchActivities();
